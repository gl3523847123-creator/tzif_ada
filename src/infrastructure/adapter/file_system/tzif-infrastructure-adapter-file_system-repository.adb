...
package body TZif.Infrastructure.Adapter.File_System.Repository is
   ...
   --  Populated by discover_sources, load_source, and import_cache
   --  Exported by export_cache

   Zones : TZif.Infrastructure.Cache.Zone_Cache.Zone_Cache_Type;
   --  Thread-safe cache for parsed timezone data (TZif_Data)
   --  Populated when zones are loaded
   --  Exported/imported by export_cache/import_cache
   ...
   function Import_Cache
     (Path : Application.Port.Inbound.Import_Cache.Path_String)
      return Application.Port.Inbound.Import_Cache.Import_Cache_Result
   is
      ...
      Path_Str : constant String :=
        Application.Port.Inbound.Import_Cache.Path_Strings.To_String (Path);
   begin
      --  Check if file exists
      if not Exists (Path_Str) then
         return
           Import_Cache_Result_Package.Error
             (Infrastructure_Error, "Cache file not found: " & Path_Str);
      end if;

      --  Read and parse JSON file
      declare
         F            : File_Type;
         JSON_Content : Unbounded_String;
         Root         : JSON_Value;
      begin
         --  Read file content
         Open (F, In_File, Path_Str);
         while not End_Of_File (F) loop
            Append (JSON_Content, Get_Line (F) & ASCII.LF);
         end loop;
         Close (F);

         --  Parse JSON
         Root := Read (To_String (JSON_Content));

         --  Validate header
         if not Root.Has_Field ("header") then
            return
              Import_Cache_Result_Package.Error
                (Validation_Error, "Invalid cache file: missing header");
         end if;

         --  Parse and validate header
         declare
            Header_JSON : constant JSON_Value := Root.Get ("header");
         begin
            if not Is_Valid_Cache_Header (Header_JSON) then
               return
                 Import_Cache_Result_Package.Error
                   (Validation_Error,
                    "Invalid cache header: wrong magic or version");
            end if;
         end;

         --  Import sources and zones with metadata tracking
         declare
            Header_JSON     : constant JSON_Value := Root.Get ("header");
            Header          :
              constant Infrastructure.Cache.JSON_Serialization.Cache_Header :=
              From_JSON (Header_JSON);
            Sources_Loaded  : Natural             := 0;
            Sources_Removed : Natural             := 0;
         begin
            --  Validate expected vs actual in header (v2+)
            if Header.Version >= 2 then
               if Header.Expected_Sources /= Header.Actual_Sources then
                  Put_Line ("[WARN] Cache metadata mismatch");
                  Put_Line
                    ("[WARN] Expected sources: " &
                     Header.Expected_Sources'Image & ", Actual: " &
                     Header.Actual_Sources'Image);
                  Put_Line
                    ("[WARN] Cache file may be corrupted or incomplete");
               end if;

               if Header.Expected_Zones /= Header.Actual_Zones then
                  Put_Line ("[WARN] Cache metadata mismatch");
                  Put_Line
                    ("[WARN] Expected zones: " & Header.Expected_Zones'Image &
                     ", Actual: " & Header.Actual_Zones'Image);
                  Put_Line
                    ("[WARN] Cache file may be corrupted or incomplete");
               end if;
            end if;
            ...
            --  Import sources
            if Root.Has_Field ("sources") then
               declare
                  Sources_Array : constant JSON_Array := Get (Root, "sources");
                  Arr_Length    : constant Natural := Length (Sources_Array);
               begin
                  for I in 1 .. Arr_Length loop
                     declare
                        Source_JSON : constant JSON_Value :=
                          Get (Sources_Array, I);
                        Source      :
                          constant Domain.Value_Object.Source_Info
                            .Source_Info_Type :=
                          From_JSON (Source_JSON);
                        Source_Path : constant String     :=
                          Domain.Value_Object.Source_Info.To_String
                            (Domain.Value_Object.Source_Info.Get_Path
                               (Source));
                     begin
                        --  Validate source path exists
                        if Exists (Source_Path) then
                           --  Validate version against IANA releases
                           --  and filesystem
                           declare
                              use TZif.Domain.Value_Object.IANA_Releases;
                              Cached_Version : constant Version_String_Type :=
                                Get_Version (Source);
                              Cached_Zone_Count : constant Natural :=
                                Get_Zone_Count (Source);
                              Is_Known          : constant Boolean :=
                                Is_Known_Version (Cached_Version);
                           begin
                              --  Check if this is an unknown version
                              if not Is_Known then
                                 --  Unknown version - log warning but proceed
                                 Put_Line
                                   ("[WARN] Unknown IANA version in cache: " &
                                    To_String (Cached_Version) & " for " &
                                    Source_Path);
                                 Put_Line
                                   ("[WARN] Proceeding with cached " &
                                    "zone count, but validation skipped");
                              else
                                 --  Known version - validate zone count
                                 declare
                                    Expected_Zone_Count : constant Natural :=
                                      Get_Expected_Zone_Count (Cached_Version);
                                 begin
                                    if Expected_Zone_Count > 0
                                      and then Cached_Zone_Count /=
                                        Expected_Zone_Count
                                    then
                                       --  Zone count mismatch - log warning
                                       --  but proceed
                                       Put_Line
                                         ("[WARN] Zone count mismatch for " &
                                          To_String (Cached_Version));
                                       Put_Line
                                         ("[WARN] Expected: " &
                                          Expected_Zone_Count'Image &
                                          " zones");
                                       Put_Line
                                         ("[WARN] Cached: " &
                                          Cached_Zone_Count'Image & " zones");
                                       Put_Line
                                         ("[WARN] Path: " & Source_Path);
                                       Put_Line
                              ...
                              --  Scenario 2: Check for version mismatch
                              --  with filesystem. Read the +VERSION file
                              --  from the directory
                              declare
                                 use type Version_String_Type;
                                 Dir_Path     : constant String :=
                                   Containing_Directory (Source_Path);
                                 Version_File : constant String :=
                                   Dir_Path & "/+VERSION";

                                 function Read_Filesystem_Version return String
                                 is
                                    File : File_Type;
                                    Line : String (1 .. 32);
                                    Last : Natural;
                                 ...
                                 exception
                                    when others =>
                                       return "unknown";
                                 end Read_Filesystem_Version;

                                 Filesystem_Version :
                                   constant Version_String_Type :=
                                   Make_Version (Read_Filesystem_Version);
                              begin
                                 if Cached_Version /= Filesystem_Version then
                                    --  Version mismatch â ERROR
                                    Put_Line
                                      ("[ERROR] Version mismatch detected");
                                    Put_Line
                                      ("[ERROR] Cached version: " &
                                       To_String (Cached_Version));
                                    Put_Line
                                      ("[ERROR] Filesystem version: " &
                                       To_String (Filesystem_Version));
                                    Put_Line ("[ERROR] Path: " & Source_Path);
                                    Put_Line
                                      ("[ERROR] Consider re-discovering " &
                                       "sources or clearing cache");
                                    return
                                      Import_Cache_Result_Package.Error
                                        (Infrastructure_Error,
                              ...
                              --  Create Source_Entry with canonical path
                              declare
                                 use TZif.Infrastructure.Cache.Source_Cache;
                                 Source_Path_Type :
                                   constant Path_String_Type :=
                                   Get_Path (Source);
                                 --  Canonicalize: resolve symlinks +
                                 --  normalize separators/case
                                 Canonical : constant Path_String_Type :=
                                   Infrastructure.Cache.Path_Canonical
                                     .Canonicalize
                                     (Make_Path (Full_Name (Source_Path)));
                                 Source_Entry_Val :
                                   constant Infrastructure.Cache.Source_Cache
                                     .Source_Entry :=
                                   Make_Source_Entry (Source, Canonical);
                              begin
                                 --  Duplicate detection: Check if path
                                 --  or canonical already exists
                                 declare
                                    Exact_Duplicate     : constant Boolean :=
                                      Sources.Contains (Source_Path_Type);
                                    Canonical_Duplicate : Boolean := False;
                                 begin
                                    --  Check for canonical path
                                    --  duplicates (symlinks)
                                    if not Exact_Duplicate then
                                       --  Get all sources and check
                                       --  canonical paths
                                       declare
                                          All_Sources :
                                            constant Source_Map_Type :=
                                            Sources.Get_All;
                                          Cursor      : Source_Maps.Cursor :=
                                            All_Sources.First;
                                          use Source_Maps;
                                          use type Path_String_Type;
                                          --  Make "=" visible
                                       begin
                                          while Has_Element (Cursor) loop
                                             declare
                                                Existing_Entry :
                                                  constant Source_Entry :=
                                                  Element (Cursor);
                                             begin
                                                if Existing_Entry
                                                    .Canonical_Path =
                                    ...
                                    --  Scenario 1: Exact path
                                    --  duplicate â ERROR
                                    if Exact_Duplicate then
                                       Put_Line
                                         ("[ERROR] Duplicate source " &
                                          "path in cache: " &
                                          To_String (Source_Path_Type));
                                       Put_Line
                                         ("[ERROR] Cache integrity " &
                                          "violation - same path cannot " &
                                          "be added twice");
                                       return
                                         Import_Cache_Result_Package.Error
                                           (Infrastructure_Error,
                                            "Duplicate source path in cache:" &
                                            To_String (Source_Path_Type));

                                       --  Scenario 4: Symlink to same
                                       --  canonical path â ERROR
                                    elsif Canonical_Duplicate then
                                       Put_Line
                                         ("[ERROR] Canonical path already " &
                                          "exists in cache");
                                       Put_Line
                                         ("[ERROR] Attempted: " &
                                          To_String (Source_Path_Type));
                                       ...
                                       Sources.Insert
                                         (Source_Path_Type, Source_Entry_Val);
                           ...
                           --  Path doesn't exist, skip this source
                           Sources_Removed := Sources_Removed + 1;
                        end if;
                     end;
                  end loop;
               end;
            ...
            begin
               if Root.Has_Field ("zones") then
                  declare
                     Zones_Array : constant JSON_Array := Get (Root, "zones");
                     Arr_Length  : constant Natural    := Length (Zones_Array);
                  begin
                     for I in 1 .. Arr_Length loop
                        declare
                           Zone_JSON      : constant JSON_Value     :=
                             Get (Zones_Array, I);
                           Zone_ID_Str    : constant String         :=
                             Zone_JSON.Get ("zone_id");
                           Zone_ID        : constant Zone_Id_Type   :=
                             Make_Zone_Id (Zone_ID_Str);
                           Zone_Data_JSON : constant JSON_Value     :=
                             Zone_JSON.Get ("data");
                           Zone_Data      : constant TZif_Data_Type :=
                             From_JSON (Zone_Data_JSON);
                        begin
                           --  Add to zone cache
               ...
               --  Validate loaded counts against header expectations (v2+)
               if Header.Version >= 2 then
                  if Sources_Loaded /= Header.Actual_Sources then
                     Put_Line ("[WARN] Loaded source count mismatch");
                     Put_Line
                       ("[WARN] Header claimed: " &
                        Header.Actual_Sources'Image);
                     Put_Line
                       ("[WARN] Actually loaded: " & Sources_Loaded'Image);
                  end if;

                  if Zones_Loaded /= Header.Actual_Zones then
                     Put_Line ("[WARN] Loaded zone count mismatch");
                     Put_Line
                       ("[WARN] Header claimed: " & Header.Actual_Zones'Image);
                     Put_Line
                       ("[WARN] Actually loaded: " & Zones_Loaded'Image);
                  end if;
               end if;
               ...
               begin
                  return Import_Cache_Result_Package.Ok (Stats);
               end;
            end;
         end;

      exception
         when Ada.Text_IO.Name_Error | Ada.Directories.Name_Error =>
            return
              Import_Cache_Result_Package.Error
                (Infrastructure_Error, "Cache file not found: " & Path_Str);
         when E : others                                          =>
   ...
   function Export_Cache
     (Path      : TZif.Application.Port.Inbound.Export_Cache.Path_String;
      Overwrite : Boolean)
      return Application.Port.Inbound.Export_Cache.Export_Cache_Result
   is
      ...
      Path_Str : constant String :=
        Application.Port.Inbound.Export_Cache.Path_Strings.To_String (Path);
   begin
      --  Check if file exists and overwrite not allowed
      if Exists (Path_Str) and then not Overwrite then
         return
           Export_Cache_Result_Package.Error
             (Validation_Error,
              "File exists and overwrite not allowed: " & Path_Str);
      end if;

      --  Get all sources and zones from cache
      declare
         Source_Map   :
           constant Infrastructure.Cache.Source_Cache.Source_Map_Type :=
           Sources.Get_All;
         Source_Count : constant Natural := Natural (Source_Map.Length);

         Zone_Map   :
           constant Infrastructure.Cache.Zone_Cache.Zone_Data_Map_Type :=
           Zones.Get_All;
         Zone_Count : constant Natural := Natural (Zone_Map.Length);
         ...
         Header_Obj    :
           ...
           (Magic            => "TZIF_CACHE",
            ...
            Expected_Zones   => Zone_Count,
            Actual_Sources   => Source_Count,
            Actual_Zones     => Zone_Count);
         Sources_Array : constant JSON_Value := Create (Empty_Array);

         use TZif.Infrastructure.Cache.Source_Cache.Source_Maps;
      begin
         --  Add header
         Root.Set_Field ("header", To_JSON (Header_Obj));

         --  Add sources
         for Cursor in Source_Map.Iterate loop
            declare
               use TZif.Infrastructure.Cache.Source_Cache;
               Entry_Val   : constant Source_Entry := Element (Cursor);
               --  Convert Source_Entry back to Source_Info for JSON
               Source_Info :
                 constant Domain.Value_Object.Source_Info.Source_Info_Type :=
                 Domain.Value_Object.Source_Info.Make_Source_Info
                   (ULID       => Entry_Val.ULID, Path => Entry_Val.Path,
                    Version    => Entry_Val.Version,
                    Zone_Count => Entry_Val.Zone_Count);
            begin
               Append (Sources_Array, To_JSON (Source_Info));
            end;
         end loop;
         Root.Set_Field ("sources", Sources_Array);

         --  Add zones
         declare
            Zones_Array : constant JSON_Value := Create (Empty_Array);
            use TZif.Infrastructure.Cache.Zone_Cache.Zone_Data_Maps;
         begin
            for Cursor in Zone_Map.Iterate loop
               declare
                  use TZif.Infrastructure.Cache.Zone_Cache;
                  Zone_ID    : constant Zone_Id_Type     := Key (Cursor);
                  Zone_Entry : constant Zone_Cache_Entry := Element (Cursor);
                  Zone_Data  : constant TZif_Data_Type   := Zone_Entry.Data;
                  Zone_Obj   : constant JSON_Value       := Create_Object;
               begin
                  --  Each zone entry has: zone_id and data
                  Zone_Obj.Set_Field ("zone_id", Create (To_String (Zone_ID)));
                  Zone_Obj.Set_Field ("data", To_JSON (Zone_Data));
                  Append (Zones_Array, Zone_Obj);
               end;
            end loop;
            Root.Set_Field ("zones", Zones_Array);

            --  Write to file
            declare
               F : File_Type;
            begin
               Create (F, Out_File, Path_Str);
               Put_Line (F, Write (Root, Compact => False));  --  Pretty JSON
               Close (F);
            exception
               when E : others =>
                  if Is_Open (F) then
            ...
            declare
               Stats : constant Export_Stats_Type :=
                 (Sources_Exported => Source_Count,
                  Zones_Exported   => Zone_Count);
            begin
               return Export_Cache_Result_Package.Ok (Stats);
            end;
         end;
      end;
   exception
      when E : others =>
         return
           Export_Cache_Result_Package.Error
             (Infrastructure_Error,
              "Cache export failed: " & Ada.Exceptions.Exception_Message (E));
   end Export_Cache;
